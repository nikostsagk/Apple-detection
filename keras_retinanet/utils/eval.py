...
def evaluate(
    ...
    for label in range(generator.num_classes()):
        ...
        false_positives[label] = np.zeros((0,))
        ...
        for i in range(generator.size()):
            ...
            for d in detections:
                ...
                if annotations.shape[0] == 0:
                    ...
                    true_positives[label]  = np.append(true_positives[label], 0)
                    continue
        ...
        false_positives[label] = false_positives[label][indices]
        true_positives[label]  = true_positives[label][indices]

        # compute false positives and true positives
        false_positives[label] = np.cumsum(false_positives[label])
        true_positives[label]  = np.cumsum(true_positives[label])

        # compute recall and precision
        recall    = true_positives[label] / num_annotations
        precision = true_positives[label] / np.maximum(true_positives[label] + false_positives[label], np.finfo(np.float64).eps)
